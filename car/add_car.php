...
<div class="row">
          <div class="col-lg-5">
			...
			<b>ทะเบียนรถยนต์</b>
                        <input type='text' name='license'  size="4"  id='license' placeholder='ใส่ทะเบียนรถยนต์' class='form-control' value='<?= isset($resultGet['license_name'])?$resultGet['license_name']:''?>' required>
			 </div> 
                        <div class="form-group">
                            <b>ประเภทรถยนต์ </b>
          ...
          if($result['oil_id']==$resultGet['oil_type']){$selected='selected';}else{$selected='';}
				 echo "<option value='".$result['oil_id']."' $selected>".$result['oil_name'] ."</option>";
                        ...
                        <div class="form-group">
                            <?php if(!empty($method) and $method=='update'){
 			$take_date= $resultGet['recive_date'];
                        }else{
                         $take_date=date('Y-m-d');  
                        }
 		?>
                            <b>วันที่รับรถ </b>
                            <input type="text" id="datepicker" name="recive_date" class="form-control" value="<?= edit_date($take_date)?>">
                        </div>
                        <div class="form-group">
                            <b>สถานะรถยนต์ </b>
