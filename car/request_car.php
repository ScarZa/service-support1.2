              ...
              <div class="panel panel-warning">
                ...
                <div class="panel-body">
                    ...
                    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                                    ...
                                    ฝ่าย/งาน/กลุ่มงาน <b><?php if($method=='edit') { echo $edit_person['depName'];}else{ echo $resultDep['depName'];}?></b> ขออนุญาตใช้รถราชการ<p>
<?php }else{?>
				...
				<?php	$sql = mysqli_query($db,"SELECT concat(firstname,' ',lastname) as fullname, empno  FROM emppersonal 
                                            order by firstname");
				 echo "<option value=''>--เลือกผู้ขอใช้รถยนต์--</option>";
          ...
          if($result['empno']==$edit_person['empno_request']){$selected='selected';}else{$selected='';}
				 echo "<option value='".$result['empno']."' $selected>".$result['fullname'] ."</option>";
                    ...
                    <div class="form-group" > 
                        ...
                        <input value="<?= isset($edit_person['place'])?$edit_person['place']:''?>" NAME="place" id="place"  class="form-control" onkeydown="return nextbox(event, 'start_date')" placeholder="สถานที่ที่ต้องไป" size="80" required>
                    </div><p>
                        <div class="form-group" > 
                ...
                <?php if(!empty($method)){
 			$take_date= $edit_person['start_date'];
                        $take_date1 = $edit_person['end_date'];
                        }else{
 		...
 		?>
                ...
                <input name="start_date" type="text" id="datepicker" value="<?= edit_date($take_date)?>" placeholder='รูปแบบ 2016-01-31' class="form-control" required>
                    </div>
                <div class="form-group">
                    <label for="end_date">ถึงวันที่ &nbsp;</label>
                    <input name="end_date" type="text" id="datepicker1" value="<?= edit_date($take_date1)?>" placeholder='รูปแบบ 2016-01-31' class="form-control" required>
                </div><p>
                <div class="row">  
                <div class="form-group col-lg-4 col-md-5 col-xs-12">  <label for="take_hour_st">ตั้งแต่&nbsp;</label>  
                <div class="form-group sm"> 
                <select name="take_hour_st" id="take_hour" class="form-control select2" style="width: 100%" required>
                    ...
                    <div class="form-group">
                <select name="take_minute_st" id="take_minute" class="form-control select2" style="width: 100%" required>
                ...
                <div class="form-group"> 
                <select name="take_hour_en" id="take_hour" class="form-control select2" style="width: 100%" required>
                    <option value="">ชั่วโมง</option>
                    <?php for($i=0;$i<=23;$i++){
                    ...
                    <div class="form-group"> 
                        <select name="take_minute_en" id="take_minute" class="form-control select2" style="width: 100%" required>
                    <option value="">นาที</option>
                    <?php for($i=0;$i<=59;$i++){
                    ...
                    if($i<10){
                     ...
                     <div class="form-group">
                        ...
                        <input name="amount_date" id="amount_date" type="text" value="<?= isset($edit_person['amount_date'])?$edit_person['amount_date']:''?>" size="1" onkeyup="javascript:inputDigits(this);" class="form-control" placeholder='จำนวนวัน'>
                        <font color="red"><b>** หากไม่ถึงครึ่งวัน ไม่ต้องใส่</b></font>
                     </div>
                    <p>
                <div class="form-group" > 
                        <label for="obj">เพื่อ &nbsp;</label>
                        <input value="<?= isset($edit_person['obj'])?$edit_person['obj']:''?>" NAME="obj" id="obj"  class="form-control" onkeydown="return nextbox(event, 'start_date')" placeholder="วัตถุประสงค์" size="80" required>
                    </div><p>
                    <div class="form-group">
                        <label for="amount">จำนวนผู้ร่วมทาง</label>
                        <input name="amount" id="amount" type="number" value="<?= isset($edit_person['amount'])?$edit_person['amount']:''?>" required="" size="1" class="form-control" placeholder='จำนวนคน'>
                    </div>
                    <p>
                    <div class="form-group">
          ...
          if($result['empno']==$edit_person['passenger']){$selected='selected';}else{$selected='';}
				 echo "<option value='".$result['empno']."' $selected>".$result['fullname'] ."</option>";
                    ...
                    <?php if(isset($edit_person['wait'])?$edit_person['wait']:''=='Y') {
                        $checked='checked';
                        $check='';
                    }else{ $checked='';
                           $check='checked';
